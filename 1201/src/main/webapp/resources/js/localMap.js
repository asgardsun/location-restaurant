...
// 마커 출력
$.getJSON('localRestProc.jsp', function(data) {
    ...
    $.each(data, function(index, item) {
		...
		iwContent = '<div class="info-title">' + '<b><span style="font-size:19px;">' + item.name + '</span> </b>' 
				  + '&ensp;<span style="color:blue; font-size:13px;">' + item.contact + '</span>' + '</br>' 
				  + '<div class="line"></div><b>'
				  + item.place + '</b></br>' 
				  + '<span style="color:#56721c;"><b>' + item.main_menu +'</b></span><br>'
...
var cb = document.getElementById("chungBuk"); //충북 괴산군청 좌표
cb.onclick = function seoulTo() {
    var moveLatLon = new kakao.maps.LatLng(36.81529924821332, 127.78665650858851);
    map.panTo(moveLatLon);            
}
...
var jb = document.getElementById("jeonBuk"); //전북 임실군청
jb.onclick = function seoulTo() {
    var moveLatLon = new kakao.maps.LatLng(35.6179784136882, 127.28913638459116);
    map.panTo(moveLatLon);            
}    

var jn = document.getElementById("jeonNam"); // 전남 순천역
jn.onclick = function seoulTo() {
    var moveLatLon = new kakao.maps.LatLng(34.945938484464364, 127.50324926923558);
    map.panTo(moveLatLon);            
}    

var gb = document.getElementById("gyeongBuk"); //경북 경상북도청
gb.onclick = function seoulTo() {
    var moveLatLon = new kakao.maps.LatLng(36.576289188607454, 128.5056330151452);
    map.panTo(moveLatLon);            
}
